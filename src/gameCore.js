...
export function setConfig(partial){ Object.assign(config, partial); }
export function setWords(w){ words = w || []; if(ui) ui.enableStart(true); }
...
export function retryWord(){
  ...
  if(ui) ui.updateDisplays({word: words[currentWordIndex]||'', collected, hideWordText, attemptsLeft});
}
...
export function init(opts={}){
  ...
  setupCanvas(opts.canvasId || 'gameCanvas');

  // attach keyboard handling
...
export function startGame(){
  attemptsLeft = ui ? ui.getRetriesValue() : config.retriesDefault;
  running = true;
  currentWordIndex = 0;
  nextWord();
...
function nextWord(){
  ...
  if(currentWordIndex >= words.length){
    if(ui) ui.showOverlay('All done! Great job!', 'success');
    running = false;
    return;
  }
  attemptsLeft = ui ? ui.getRetriesValue() : config.retriesDefault;
...
function attemptCollect(){
  ...
  if(tile.char.toLowerCase() === nextChar.toLowerCase()){
    ...
    if(collected.length >= currentWord.length){
      playSuccessTune();
      if(ui) ui.showOverlay('✔️ Great! Next word', 'success');
      // advance index but keep the next word hidden until the player interacts
      currentWordIndex++;
      hideWordText = true;
      if(ui) ui.updateDisplays({word: words[currentWordIndex]||'', collected: '', hideWordText, attemptsLeft});
    ...
    if(attemptsLeft <= 0){
      playSad();
      if(ui) ui.showOverlay('✖️ Out of attempts — moving on', 'fail');
      currentWordIndex++;
      hideWordText = true;
      if(ui) ui.updateDisplays({word: words[currentWordIndex]||'', collected: '', hideWordText, attemptsLeft});
      ...
      if(ui) ui.showOverlay('Try again', 'fail');
      collected = '';
      prepareLevelFor(words[currentWordIndex] || '');
    }
  }
  // Reveal the current word after a non-terminal collect attempt so the child sees progress.
  hideWordText = false;
  if(ui) ui.updateDisplays({word: words[currentWordIndex]||'', collected, hideWordText, attemptsLeft});
}
...
function draw(){
  ...
  for(const t of letterTiles){
    ...
    ctx.fillRect(gx+6,gy+6,cs-16,cs-16);
    ctx.strokeStyle = '#e6d59a';
    ctx.strokeRect(gx+6,gy+6,cs-16,cs-16);
    ctx.fillStyle = '#333';
  ...
  if(underIdx !== -1){
    const gx = player.x*cs + 20;
    const gy = player.y*cs + 20;
  ...
  const px = player.x*cs + 20 + (cs-4)/2;
  const py = player.y*cs + 20 + (cs-4)/2;
  ctx.beginPath(); ctx.fillStyle = '#2b6cff'; ctx.arc(px,py,18,0,Math.PI*2); ctx.fill();
  ctx.fillStyle = '#fff'; ctx.font = '14px system-ui'; ctx.textAlign='center'; ctx.fillText('You', px, py+24);
}
